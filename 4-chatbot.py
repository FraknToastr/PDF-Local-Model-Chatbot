...
CHAT_MODEL_ID = "mistralai/Mistral-7B-Instruct-v0.2"  # local chat model
...
def load_embedder():
    ...
    model = AutoModel.from_pretrained(
        EMBED_MODEL_ID, dtype=torch.bfloat16, device_map="auto"
    )
...
def generate_answer(prompt: str):
    out = chat_pipe(prompt, max_new_tokens=500, temperature=0.2, do_sample=False)
...
top_k = st.sidebar.slider(
    "Number of sources to retrieve", min_value=1, max_value=10, value=3, step=1
)

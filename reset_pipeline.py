...
for i, c in enumerate(chunks[:3]):
    text = c.get("chunk", {}).get("text") or c.get("text")
    meta = c.get("metadata", {})
    logger.info(f"Sample chunk {i+1}: {text[:100]}...")
    logger.info(f"   Metadata: {meta}")

# --- Keyword search ---
hits = [
    c for c in chunks
    if CHECK_KEYWORD.lower() in (c.get("chunk", {}).get("text", "").lower() or c.get("text", "").lower())
]

logger.info(f"🔍 Found {len(hits)} chunks containing '{CHECK_KEYWORD}'")
if hits:
    for h in hits[:3]:
        logger.info(f"   → {h.get('chunk', {}).get('text') or h.get('text')}")
